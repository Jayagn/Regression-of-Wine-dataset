...
wine.fit5 = lm(quality ~ . - density,cleanWhiteDat)
summary(wine.fit5)
...
# Adjusted R2 = 0.3247
# MSE = 0.4414354
wine.fit6 = lm(quality ~ . - density - fixed.acidity - citric.acid ,cleanWhiteDat)
summary(wine.fit6)
plot(wine.fit6)
mean(vif(wine.fit6))
ols = ols_step_all_possible(wine.fit6)
ols <- ols
print(max(ols$adjr))
...
wine.fit4 = lm(quality ~ .- citric.acid - chlorides - density - fixed.acidity  , data = cleanWhiteDat)
summary(wine.fit4)
#Calculating the MSE
mean(wine.fit4$residuals^2)
vif(wine.fit4)

wine.pca = prcomp(cleanWhiteDat, center = TRUE , scale. = TRUE)
summary(wine.pca)
wine.pca
plot(wine.pca , type = "barplot")
library(olsrr)
ols = ols_step_all_possible(wine.fit)
ols <- ols
print(max(ols$adjr))
print(ols[which.max(ols$adjr),])
View(ols)
ols.best = ols_step_best_subset(wine.fit4)
View(ols.best)
plot(ols.best)
