...
# Adjusted R2 = 0.3247
# MSE = 0.4414354
wine.fit6 = lm(quality ~ . - density - citric.acid ,cleanWhiteDat)
summary(wine.fit6)
plot(wine.fit6)
mean(vif(wine.fit6))
##--------------------------------------2
wine.fit7 = lm(quality ~ . - density - fixed.acidity - citric.acid ,cleanWhiteDat)
summary(wine.fit7)
plot(wine.fit7)
mean(vif(wine.fit7))
##--------------------------------------3

ols = ols_step_all_possible(wine.fit7)
ols <- ols
print(max(ols$adjr))
print(ols[which.max(ols$adjr),])
ols.best = ols_step_best_subset(wine.fit7)
View(ols.best)
durbinWatsonTest(wine.fit7)########LASTT
